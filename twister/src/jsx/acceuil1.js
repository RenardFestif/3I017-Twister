...
class Main1Page extends Component {
	...
	handleConnexion(){
		if (this.props.cliquer_sur_connexion){
			this.setState(false)
	...
	render(){
		...
		<section>
			<div id="container">
				...
				<div id="Links">
					<a className = "button" onClick={()=> alert('click')}>Connexion</a>
					<a className = "button" onClick={()=> alert('click')}>Inscription</a>
				</div>

				<div id="connexion" className="modal">
					<span onClick={()=> alert('click')} className="close" title="Close Modal">&times;</span>
